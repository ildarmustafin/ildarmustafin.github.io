	...
	if (response.ok) {
		...
		for (let i = load_day_from; i <= (load_day_to-1); i++) { //26.01 - 30.01 и 01.02 - 04.02
			for (let j = 0; j <= 11; j++) {				
				...
				//if (once) xl.push(xlabel);
				//yd.push(jsonfile.days[i][j]);	
				if (once) xData.y.push(xlabel);
				xData.x.push(jsonfile.days[i][j]);	
		...
		for (let j = 0; j <= lh; j++) {                         //31.01 и 05.02
			...
			//if (once) xl.push(xlabel);
			//yd.push(jsonfile.days[load_day_to][j]);	
			if (once) xData.y.push(xlabel);
			xData.x.push(jsonfile.days[load_day_to][j]);
		}
	} 
	else {
...
async function CalculateMonthToOpen(base_numb,once) {
	...
	console.log(xData);
	console.log(base_numb);
	
	console.log(xData.x);
	console.log(xData.y);
	myChart.data.labels = [xData.x];		
	myChart.data.datasets[base_numb].data = [xData.y];
	
	
	myChart.update();	
}

function eraseAll(chart) {
	myChart.destroy();
	/*
    const newData = [];
    const newLabel = [];
