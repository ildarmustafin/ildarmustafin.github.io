...
//let test_json = "{\"now\":\"2021-03-07T16:18:59\",\"t\":-142.7,\"led\":0,\"rssi\":-89,\"bf\":255,\"heap\":17992}";
let dtnow;
let sec_left;
let nowisValid = 0;
let ws_client;
...
function getCountSec(){
	if (ota == 0) {
    	if (nowisValid != 0) {    
			...
			parsed_hour = new_date.getHours();	days
			parsed_week = new_date.getDay();
			document.getElementById("text_t3").innerHTML = new_date.toLocaleString()+" "+days[selLang][parsed_week];
	   		document.getElementById("text_t3").style="color:white;font-size:20px;line-height:20px;text-align:center;font-weight:bold;";
    		//setTimeout(function () { getCountSec();}, 986);
    	} else {
			noDate();	
		}
 	}
...
function getJson(data) {
	...
	    	sec_left = dtnow.getTime() / 1000;  		
			...
			openChart(); 
			document.getElementById("text_t3").innerHTML = dtnow.toLocaleString()+" "+days[selLang][parsed_week];
			document.getElementById("text_t3").style="color:white;font-size:20px;line-height:20px;text-align:center;font-weight:bold;";
		}
		...
		if (json.t == 850 || json.t <= -550 || json.t >= 1250 || json.t == "undefined") {
			...
			document.getElementById("text_t2").innerHTML = parseFloat(json.t / 10).toFixed(1) + "°C";
...
function openlang(lng) {
	...
	let selIndex = document.getElementById("select_schedule").options.selectedIndex;	
	selLang = document.getElementById("select_lang").options.selectedIndex;	
	let xhttp = new XMLHttpRequest();
...
function openValues() {
	...
	xhttp.onload = function () {
		if (this.readyState == 4 && this.status == 200) {
			...
			for (let j = 2; j <= 11; j++) { 	
				for (let i in configSetup.dt_addr){		
					let option = document.createElement("option");
					option.value = configSetup.dt_addr[i];
					option.text = configSetup.dt_addr[i];
					document.getElementById('input1'+(parseInt(j))).appendChild(option);
				}
				document.getElementById('input1'+(parseInt(j))).selectedIndex = configSetup.dt_select[j-1];				
			}		
			...
			for (let j = 0; j <= 3; j++) {
				...
				ttip_rel[j] = (nowisValid != 0) ? configSetup.schedule[1][daysOfw[parsed_week]][j] : "--:--";
			}
/*
			let fan_on  = parseFloat(configSetup.input[1][3]/100 + (configSetup.input[1][4] / 200)).toFixed(2);
			let fan_off = parseFloat(configSetup.input[1][3]/100 - (configSetup.input[1][4] / 200)).toFixed(2);
			let ten_off  = parseFloat(configSetup.input[1][6]/100 + (configSetup.input[1][7] / 200)).toFixed(2);
			let ten_on = parseFloat(configSetup.input[1][6]/100 - (configSetup.input[1][7] / 200)).toFixed(2);

			let ttip_temp = 'COR : '+ parseFloat(configSetup.input[1][0]/10).toFixed(2) + ' °C';
			ttip_wifi = 'RSSI: '+rssi+' dBm\n'+ 'HeapMin: ' +heap+ '\nMQTT: ' +mqtt+ ' WS: ' +ws_client+ '\nRST COUNT: ' +rst_count;			
