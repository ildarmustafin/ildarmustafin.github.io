...
const makeColor = async (num,fcolor) => {
	...
	document.getElementById("val_pwm"+num).style.background = fcolor;	
	rgb_pwm[num-1] = fcolor;
}
...
const getJson = async (jsonFile) => {
	...
	// Обработка JSON.pwm	
	for (let i = 1; i <= 6; i++) {
		document.getElementById('val_pwm' + i).innerHTML = json.pwm[i - 1] + "%";
	}
...
const openlang = async (cfgLang, countTry) => {
	//try {
		...
		for (let i = 1; i <= 6; i++){
			...
			document.getElementById("text_rel" + i).value = configLang.rel[i-1];
		}		
		...
		document.getElementById("Button42").value = but[6]; //Сохранить	
...
const openValues = async (countTry) => {
	...
	//try {	
		...
		for (let i = 0; i < 6; i++) {		
			rgb_pwm[i] = configSetup.rgb_pwm[i]; 		
			document.getElementById('text_pwm' + (i+1)).style.background = rgb_pwm[i];
			document.getElementById('val_pwm' + (i+1)).style.background = rgb_pwm[i];
			document.getElementById('text_pwm_color' + (i+1)).value = rgb_pwm[i];
		...
		for (let i = 0; i <= 1; i++) { //Чтение checkbox
			document.getElementById('chk' + i).checked = parseInt((document.getElementById('opt8').value & powC) >> (i), 10); 
...
const init = () => {
	...
	let select_smode0 = document.getElementById("select_smode0");
	let btn31 = document.getElementById("Button31");
	let select_smode1 = document.getElementById("select_smode1");
	let btn41 = document.getElementById("Button41");
	let btn42 = document.getElementById("Button42");
	let select_smode2 = document.getElementById("select_smode2");
	let select_smode3 = document.getElementById("select_smode3");	
	let select_smode4 = document.getElementById("select_smode4");
	let select_smode5 = document.getElementById("select_smode5");
	...
	selLang.addEventListener("change", function(event){ 
		event.preventDefault();
		openlang(this.value, 0);
	});

	selMonth.addEventListener("change", function(event){ 
		event.preventDefault();
...
/* 		let fan_on  = parseFloat(document.getElementById("input213").value) + (parseFloat(document.getElementById("input214").value) / 2);
	...
	btn41.addEventListener("click", function(event){ 
		...
		for (let i = 0; i <= 1; i++) {
			chk += (document.getElementById("chk" + i).checked) << (i);
		}
	...
	select_smode2.addEventListener("change", function(event){ 
		event.preventDefault();
		document.getElementById('opt6').value = document.getElementById('select_smode2').options.selectedIndex+1;
	});	
	select_smode3.addEventListener("change", function(event){ 
		event.preventDefault();
		document.getElementById('opt7').value = document.getElementById('select_smode3').options.selectedIndex+1;
	});	
	select_smode4.addEventListener("change", function(event){ 
		event.preventDefault();
		document.getElementById('opt9').value = document.getElementById('select_smode4').options.selectedIndex;
	});	
	select_smode5.addEventListener("change", function(event){ //editTextLang
		event.preventDefault();
		let boolCanEdit = document.getElementById('select_smode5').options.selectedIndex;
		let elems = document.getElementsByClassName('editText');

			for (i = 0; i < elems.length; i++) { // Все editText в режим readOnly
				elems[i].readOnly = boolCanEdit;
			}
			for (i = 1; i <= 6; i++) {
				document.getElementById('val_pwm' + i).hidden = !boolCanEdit;	
				document.getElementById('text_pwm_color' + i).hidden = boolCanEdit;	
			}
			document.getElementById('opt10').disabled = boolCanEdit;
			document.getElementById('opt11').disabled = boolCanEdit;	
	});		
	opt10.addEventListener("change", function(event){ 
		event.preventDefault();
		for (i = 1; i <= 6; i++) {
			document.getElementById('text_pwm' + i).style.color = document.getElementById('opt10').value;
			document.getElementById('val_pwm' + i).style.color = document.getElementById('opt10').value;	
		}	
	});		
	opt11.addEventListener("change", function(event){ 
		event.preventDefault();
		for (i = 1; i <= 6; i++) {
			document.getElementById('text_rel' + i).style.color = document.getElementById('opt11').value;	
		}	
	});	
